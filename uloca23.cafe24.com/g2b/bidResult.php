...
foreach($item1 as $arr ) { //foreach element in $arr
	$k = $arr['opengRank'] ; //$i+1; 낙찰순위
	
	// rmrk 에 순위와 비고 같이 들어감 -by jsj 20190731
	if ($k <> 0 && $arr['rmrk'] == '' ) {
		$Rank_rmark = (string)$k;
	} else {
		$Rank_rmark = $arr['rmrk'];
...
<div style='position: fixed; top: 115px; right: 210px;' class="btn_areas"><a onclick="locaciotn.href='https://uloca.net'" class="search">통합검색</a></div>
