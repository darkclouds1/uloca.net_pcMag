...
$openBidSeq = 'openBidSeq_'.$g2bClass->dt2duration('2018');
...
function insertSeq2x() {
	if (insIdx < noRow) // noRow
	{
		bidNtceNo = insTable.rows[insIdx].cells[1].innerHTML.split('-')[0];
			...
			url = 'dailyDataInsSeq12.php?bidNtceNo='+bidNtceNo+'&bidNtceOrd=00&openBidInfo='+openBidInfo + '&openBidSeq='+openBidSeq+'&bididx='+bididx+'&pss='+pss;
			//alert(url);
...
function movetmp() {
	url = 'dailyDataHandle12.php?openBidSeq='+openBidSeq;
			//alert(url);
...
$result = $dbConn->getBidNo($conn,$openBidSeq,$last,10);
