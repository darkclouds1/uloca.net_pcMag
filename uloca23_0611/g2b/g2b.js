...
function searchajax000() {	  	// 상세검색 (통합검색은 g2b_2019.js) 
	...
	if (userId == '' && (SearchCounts > searchCountMax))	{
		alert ("ln17::" + SearchCounts + "무료 검색 횟수가 초과 했습니다. 로그인 하십시요." ); //-by jsj 0312
	...
	else server="/datas/publicData.php/?searchType=0";	// searchType=0 :상세검색
	clog(server+'?'+parm);
...
function viewComp(bidNtceNo,bidNtceOrd,dt) {
	...
	url = '/g2b/bidWinner.php?bidNtceNo='+bidNtceNo+'&bidNtceOrd='+bidNtceOrd+'&id='+userId; ;
	//alert(url);
	popupWindow = window.open(url); //,'_blank','height=920,width=880,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes');
...
function bidInfo(compno) {
	url = '/g2b/datas/getInfobyComp.php?compno='+compno+'&id='+userId;
...
function searchcomp() {
	...
	//return;
	url = '/g2b/datas/bidCompanySearch.php?compname='+encodeURIComponent(compname)+'&compno='+encodeURIComponent(compno)+'&duration='+encodeURIComponent(frm.duration.value)+'&id='+userId;;
	move();
