...
function searchBidNoList(){
	...
	//---------------------------------------------------------
	isRun = true;
	url = "./wp-content/plugins/g2b/updateBidNoList.php?bidNoList=" + bidNoList + "&userId=" + userId + "&type=U";	// 관심 입찰번호, id
...
function saveCookieBidNo(obj, bidNo){
	...
	if ($(obj).prop("checked") == true) {
		if (cookieBidNoList.indexOf(trim(bidNo)) == -1) {
			cookieBidNoList.push(bidNo); // 공고 추가
		}
	} else { 
		if (cookieBidNoList.indexOf(trim(bidNo)) != -1) {
...
function makeLink_cookieKwd (cname, cookieArr){
	...
	var idxUndefined = cookieArr.indexOf("undefined");
	if (idxUndefined != -1) {
		cookieArr.splice(idxUndefined,1);
	}

	var lastIdx = cookieArr.length - 1;
	for (var i in cookieArr){
		var cntNum = i; cntNum++;
		switch (cname) {
			case 'kwd':
				cookieLink += '<a onclick=\'viewKwd(\"' + cookieArr[i] + '")\'>' + cntNum + ")" + cookieArr[i] + '</a> ';
				cookieLink += "<a onclick=\"delKwd(\'kwd\', 1 ,\'" + cookieArr[i] + "\',\'" + cookieArr + "\')\;\"> x</a>&nbsp&nbsp";
				if (i == cookieArr.length -1 ) {				
					if (cookieArr.length != 1 ) { 
						cookieLink += "&nbsp<a onclick=\"delKwd(\'kwd\',0)\;\">→삭제</a>";
					}
				}
				break;

			case 'compName':
				cookieLink += '<a onclick=\'viewCompKwd(\"' + cookieArr[i] + '")\'>' + cntNum + ")" + cookieArr[i] +  '&nbsp</a> ';
				cookieLink += "<a onclick=\"delKwd(\'compName\', 1 ,\'" + cookieArr[i] + "\',\'" + cookieArr + "\')\;\"> x</a>&nbsp&nbsp&nbsp";
				if (i == cookieArr.length -1 ) {
					if (cookieArr.length != 1 ) { 
						cookieLink += "&nbsp<a onclick=\"delKwd(\'compName\',0)\;\">→삭제</a>";
					}
				}
				break;
		}
