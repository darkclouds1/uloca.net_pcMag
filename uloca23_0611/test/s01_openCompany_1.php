...
function companyInfoUpdate($conn, $g2bClass, $str, $end) {
	...
	echo ("-----------------------------------------------------------------------------------------");
	echo ("<br> 전체시작 ==>startDate: ".$strDate." ~ ".$endDate);
	...
	// ---- 결과 표시
	echo ("<br>PageNo: [".$pageNo."] RESULT--> INSERT:[".$rtn[0]."] InsertEmail:[".$rtn[4]."], UPDATE:[".$rtn[2]."], updateEmail:[".$rtn[6]."]<br>");
	echo ("<br>------------------------------------------------------------------------------------------");
...
function updateCompInfo($conn, $item0, $pageNo, $lastPageNo, $numOfRows ) {
	...
	echo("<br>----------------------------------------------------------------");
	echo("<br> DB-Update결과 ==>  page/LastPage:[ ".$pageNo."/".$lastPageNo." ], cnt/numOfRows:[ ".($i1+1)."/".$numOfRows." ]<br>");
...
define("searType", 2); 		// 1:등록일기준, 2:변경일기준, 3:사업자등록번호
companyInfoUpdate($conn, $g2bClass, "20190101", "20191231");
