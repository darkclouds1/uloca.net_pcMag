...
class dbconn {
	...
	// 상세검색과 같이 사용 -by jsj 20200607
	//-------------------------------------
	function getSvrDataDB4($conn, $kwd, $dminsttNm, $curStart, $cntonce, $searchType) {	// sType = 0: 통합검색
		...
		if($searchType == 0 ) {
				...
				// $sql .= " AND dminsttNms like '%".$dminsttNms."%' "; 
		}
	...
	function getMemberFee($conn, $userId) {
		...
		return $loginSW; // 1: 유료회원 & 프리패스 , 0: not login
	}
