                    ...
                    <script>
                        ...
                        function init() {
                            ...
                            loginSW = <?= $loginSW ?>; cookieKwd = unescape(getCookieArray('kwd', cookieCnt));
                                ...
                                makeLink_cookieKwd('kwd', cookieKwd); // 링크 표시 쿠키 기업검색 cookieCompName.length= 0;
                                // 초기화 cookieCompName = unescape(getCookieArray('compName', cookieCnt));
                                cookieCompName = cookieCompName.split(',');
                                document.getElementById('compKwd').innerHTML = makeLink_cookieKwd('compName',
                                cookieCompName);;
                                //--------------------------------------------------------------------------------------
                                // DB->쿠키로 저장 후 관심 자동 조회 됨, 로그인 상태로 id가 있는 경우만 해당, type: R=읽기
                                //--------------------------------------------------------------------------------------
                                cookieUse = false; // 최초 자동 검색은 쿠키 저장 없음. (입찰정보 쿠키에 적용되는 것을 막기 위함) isRun = true;
                                url = "./wp-content/plugins/g2b/updateBidNoList.php?bidNoList=" +
                                cookieBidNoList + "&userId=" + userId + "&type=R"; // 관심 입찰번호, id getAjax(url,
                                updateDbBidNoList); console.log(url); } cookieUse = true; var selcolor =
                                '#eeeeee'; var unselcolor = '#ffffff'; var choiceTable = ''; function chBack(ln) {
                                if (platForm == "mobile") {
