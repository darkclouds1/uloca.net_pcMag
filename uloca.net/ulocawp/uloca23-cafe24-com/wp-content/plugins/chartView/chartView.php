<?php
/*
Plugin Name: chartView : 특정업체 응찰정보
Plugin URI: http://uloca23.cafe24.com/chartView/chartView.php
Description: 워드프레스 특정업체 응찰정보 입니다.
Version: 1.0
Author: Monolith
Author URI: http://uloca23.cafe24.com/chartView/chartView.php
*/

function chartViewShortCode() {
...
// --------------------------------- log
$rmrk = 'chartView'; // .$_SERVER ['HTTP_USER_AGENT'];
$dbConn->logWrite2($_SESSION['current_user']->user_login,$_SERVER['REQUEST_URI'],$rmrk,'','05');
// --------------------------------- log
...
function doplus() {
	...
	if (curpos == 0)
	{
		lowidx[curpos] = eval(orgdata[fridx]['tr']).toFixed(0);
		hiidx[curpos] = eval(orgdata[toidx]['tr']).toFixed(0);
	} else if (curpos == 1)
	{
		lowidx[curpos] = eval(orgdata[fridx]['tr']).toFixed(1);
		hiidx[curpos] = eval(orgdata[toidx]['tr']).toFixed(1);
	} else if (curpos == 2)
	{
		lowidx[curpos] = eval(orgdata[fridx]['tr']).toFixed(2);
		hiidx[curpos] = eval(orgdata[toidx]['tr']).toFixed(2);
	} else if (curpos == 3)
	{
		lowidx[curpos] = eval(orgdata[fridx]['tr']).toFixed(3);
		hiidx[curpos] = eval(orgdata[toidx]['tr']).toFixed(3);
	} 
	document.getElementById('frrange').value = lowidx[curpos]; //orgdata[fridx]['tr'];
	document.getElementById('torange').value = hiidx[curpos]; //orgdata[toidx]['tr'];
...
function makeparm() {
	...
	parm +='&resudi='+resudi;
	parm +='&kwd='+document.getElementById('kwd').value;
	parm +='&dminsttNm='+document.getElementById('dminsttNm').value;
	parm += '&id'+resudi;
...
function drawchart(datas) {
	...
	if (data.length<1)
	{
		...
		chartbox.innerHTML = '해당 데이타가 없습니다.';
		return;
	}
	mydrawChart();
}
...
<div class="detail_search" >
	...
	<table align=center cellpadding="0" cellspacing="0" width="90%" id='choice'>
		...
		<tr>
			...
			<input class="input_style2" style="text-align:center;" type="text" name="frrange" id="frrange" value="<?=$frrange?>" size=7> ~ 
			<!-- ? if ($mobile == 'Mobile') echo '<br>&nbsp;&nbsp;'; ? -->
			
			<input class="input_style2" style="text-align:center;" type="text" name="torange" id="torange" value="<?=$torange?>" size=7>
		...
		<tr>
				<th>공고명</th>
				<td>&nbsp;
					<input class="input_style2" autocomplete="on" type="text" name="kwd" id="kwd" style="ime-mode:active;" value="" onkeypress="if(event.keyCode==13) {searchajax(); return false;}" onclick="chBack(1)" maxlength="50" style="width:80%;" />
					<? if ($mobile == 'Mobile') echo '<br>'; ?>
					 &nbsp;&nbsp;→ 중복검색은+❝ % ❞  (ex.정보시스템 % 감리)
				</td>
			</tr>
			<tr>
				<th>수요기관</th>
				<td>&nbsp;
					<input class="input_style2" autocomplete="on" type="text" name="dminsttNm" id="dminsttNm" style="ime-mode:active;" value="" onkeypress="if(event.keyCode==13) {searchajax(); return false;}" onclick="chBack(1)" maxlength="50" style="width:80%;" />
				</td>
			</tr>
		<tr>
		...
		<div class="btn_area">
			<input class="btnorange" type="button" onclick="refresh()" value="검색">
			<input class="btnorange" type="button" onclick="reinit()" value="초기화">
			<? if ($mobile == "Mobile") echo '<p style="line-height: 1px; "><br></p>'; ?>
	  ...
	  <!-- li>응찰건수와 낙찰 상관관계</li -->
<!-- ------------------------------------- chart ----------------------------------------------------- -->
	<div style="overflow:auto; width:100%; height:280px; padding:10px; background-color:#eeeeee;">
		<div id="chartbox" style="width:100%;height:260px;border:1px solid #c0c0c0;"></div>
	</div>
<!-- ------------------------------------- chart ----------------------------------------------------- -->
...
add_shortcode('chartView','chartViewShortCode');
